...
DATA_ROOT=/home/renyong/ECCV24_NeST/data/PascalVOC12
...
OPTIONS="--backbone resnet101 --checkpoint checkpoints/step/ --fix_bachbone --fix_pre_cls"
...
FIRSTMODEL=/home/renyong/ECCV24_NeST/checkpoints/step/${TASK}-${DATASET}_FT_0.pth
...
CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --overlap --batch_size ${BATCH_SIZE} --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 0 --lr 0.02 --epochs ${INITIAL_EPOCHS} --method ${METHOD} --opt_level O1 ${OPTIONS} --ckpt ${NEW_FIRSTMODEL} --test
CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --overlap --batch_size ${BATCH_SIZE} --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 1 --lr 0.001 --epochs ${EPOCHS} --method ${METHOD} --opt_level O1 ${OPTIONS} --step_ckpt ${NEW_FIRSTMODEL}
