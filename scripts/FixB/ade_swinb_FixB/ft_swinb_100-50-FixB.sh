...
DATASET=ade
TASK=100-50
NAME=FT-SwinB-FixB-${LR}
METHOD=FT 
# relevant parameters are set here
OPTIONS="--backbone swin_b --checkpoint /mnt/gpfs/renyong/CSS/checkpoints/step/ --fix_backbone"
...
if [ -f /mnt/gpfs/renyong/CSS/checkpoints/step/${TASK}-${DATASET}_FT-SwinB-FixB-0.02_0.pth ]; then
    echo "Task 0 checkpoint already exists, skipping ..."
    cp /mnt/gpfs/renyong/CSS/checkpoints/step/${TASK}-${DATASET}_FT-SwinB-FixB-0.02_0.pth /mnt/gpfs/renyong/CSS/checkpoints/step/${TASK}-${DATASET}_${NAME}_0.pth
