...
readonly DATASET="voc"
readonly NUM_STEPS=12
readonly TASK="10-1"
readonly NAME="FT-SwinB-FixB"
readonly METHOD="FT"
readonly BACKBONE="swin_b"
readonly LR="0.001"
readonly EPOCHS=30
...
for step in $(seq 0 $((NUM_STEPS - 1))); do
    ...
    log_file="${LOG_DIR}/$(date +"%Y%m%d_%H%M%S")_${DATASET}_${TASK}_${LR}-probing_${BACKBONE}_step_${step}.log"
    ...
    if [ "${step}" -gt 0 ]; then
        sleep 5
        prev_step=$((step - 1))
        checkpoint_path="${CHECKPOINT_BASE_DIR}/${TASK}-${DATASET}_${METHOD}-SwinB-0.001_${prev_step}.pth"
