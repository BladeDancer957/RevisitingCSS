...
DATASET=ade
TASK=100-50
NAME=FT-SwinB-FixBC-${LR}
METHOD=FT 
# relevant parameters are set here
OPTIONS="--backbone swin_b --checkpoint /mnt/gpfs/renyong/CSS/checkpoints/step/ --fix_bachbone --fix_pre_cls "
...
if [ -f /mnt/gpfs/renyong/CSS/checkpoints/step/${TASK}-${DATASET}_FT-SwinB-FixBC-0.02_0.pth ]; then
    echo "Task 0 checkpoint already exists, skipping ..."
    cp /mnt/gpfs/renyong/CSS/checkpoints/step/${TASK}-${DATASET}_FT-SwinB-FixBC-0.02_0.pth /mnt/gpfs/renyong/CSS/checkpoints/step/${TASK}-${DATASET}_${NAME}_0.pth
