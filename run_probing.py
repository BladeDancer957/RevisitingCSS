...
from train_probing import Trainer
...
def run_step(opts, world_size, rank, device):
    ...
    # opts.inital_nb_classes = [21] 
    # opts.inital_nb_classes = [151] 
    opts.inital_nb_classes = [opts.num_classes] 

    step_checkpoint = None
    if opts.pre_allocate:
        assert opts.task_num > 1
        model = make_model(opts, classes=opts.inital_nb_classes, all_tasks_classes=tasks.get_per_task_classes(opts.dataset, opts.task, opts.task_num - 1))
    else:
    ...
    # if True:
        ...
        if os.path.exists(path):
            ...
            # 获取 model_state 中的 body 部分权重
            body_state_dict = {k.replace('module.body.', ''): v for k, v in step_checkpoint['model_state'].items() if k.startswith('module.body.')}

            # 将 body 部分权重加载到 model.module.body 中
            model.module.body.load_state_dict(body_state_dict)

            # 获取 model_state 中的 head 部分权重
            head_state_dict = {k.replace('module.head.', ''): v for k, v in step_checkpoint['model_state'].items() if k.startswith('module.head.')}

            # 将 head 部分权重加载到 model.module.head 中
            model.module.head.load_state_dict(head_state_dict)
    ...
    trainer = Trainer(
        ...
        classes=opts.inital_nb_classes,
        step=opts.step
    )
    ...
    if True: #TRAIN:
        ...
        if opts.pre_allocate:
            assert opts.task_num > 1
            model = make_model(
                opts, classes=opts.inital_nb_classes, all_tasks_classes=tasks.get_per_task_classes(opts.dataset, opts.task, opts.task_num - 1)
            )
        else:
            model = make_model(
                opts, classes=opts.inital_nb_classes
            )
        # Put the model on GPU
