...
class Trainer:
    ...
    def before_prototype(self, loader):
        if self.proto:
        ...
        if self.fix_pre_cls and self.step > 0:
            for i in range(self.step):
                if not self.use_cosine:
                    ...
                    model.module.cls[i].eval()
        # if self.fix_bachbone:  # for probing
        if self.fix_bachbone and self.step > 0:
            print(f"Fixing backbone for {self.step} steps")
    ...
    def update_prototypes(self, loader, device):
        """Find the median prediction score per class with the old model.
        ...
        model = self.model
        model.eval()
        # prototypes = torch.zeros([self.opts.num_classes, self.feat_dim])
        prototypes = torch.zeros([self.opts.num_classes, 256])
        prototypes.requires_grad = False
        prototypes = prototypes.to(device)
        count_features = torch.zeros([self.opts.num_classes], dtype=torch.long)
        count_features.requires_grad = False
        count_features = count_features.to(device)

        # import pdb; pdb.set_trace()
        with torch.no_grad():
            for cur_step, (images, labels) in enumerate(tqdm(loader)):
                images = images.to(device, dtype=torch.float32)
                labels = labels.to(device, dtype=torch.long)
                outputs, features = model(images, ret_intermediate=True)
                features_body = features['body'] 
                pre_logits = features['pre_logits']
                prototypes, count_features = self._update_running_stats(
                    labels,
                    (F.interpolate(input=labels.unsqueeze(dim=1).double(), size=(pre_logits.shape[2], pre_logits.shape[3]),
                    mode='bilinear', align_corners=True)).long(), 
                    pre_logits, 
                    prototypes,
                    count_features
                )
            for i in range(prototypes.shape[0]):
                print(f"num to calculate prototypes of class {i} is {count_features[i]}\n")
            print("\n\n")
        return prototypes, count_features
    def _update_running_stats(self, labels, labels_down, features, prototypes, count_features):
        cl_present_gt, _ = torch.unique(input=labels,return_counts=True)
        cl_present, _ = torch.unique(input=labels_down,return_counts=True)

        # 创建一个掩码，表示cl_present中的每个元素是否存在于cl_present_gt中
        mask = torch.isin(cl_present, cl_present_gt)

        # cl_present = torch.where(cl_present < self.opts.num_classes, cl_present, 255)
        cl_present = torch.where(mask, cl_present, 255)
        cl_present = torch.unique(input=cl_present)
        if cl_present[-1] == 255:
            cl_present = cl_present[:-1]
        #print(cl_present)

        # features_local_mean = torch.zeros([self.opts.num_classes, self.feat_dim], device=self.device)

        for cl in cl_present:
            features_cl = features[(labels_down == cl).expand(-1, features.shape[1], -1, -1)].view(features.shape[1],-1).detach()
            #print('features_cl', features_cl.size())
            # features_local_mean[cl] = torch.mean(features_cl.detach(), dim=-1)
            #print('features_local_mean[cl]',features_local_mean[cl].size())
